                                         ...
                                         int bounding_box_width)
{
    ...
    int num_boxes = 2 + (curand(&state) % 2);

    for(int i = 0; i < 3; i++)
    {
        if(i < num_boxes)
        {
            box_height[i] = 1 + (curand(&state) % bounding_height);
            box_width[i] = 1 + (curand(&state) % bounding_width);
            box_y[i] = (curand(&state) % (bounding_height-box_height[i]+1));
            box_x[i] = (curand(&state) % (bounding_width-box_width[i]+1));
            box_weights[i] = (2.0*(curand(&state)%2)-1.0) /
